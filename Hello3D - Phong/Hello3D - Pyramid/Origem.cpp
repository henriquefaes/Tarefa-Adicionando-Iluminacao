...
// Estrutura para armazenar os parâmetros do material
struct Material {
	struct Color {
		float r, g, b;
	};
	...
	float Ns; // Coeficiente especular
	float d;  // Opacidade
	float Ni; // Índice de refração
};

// Protótipos das funções
int setupGeometry();
int loadSimpleOBJ(string filepath, int& nVerts, glm::vec3 color = glm::vec3(1.0, 0.0, 1.0));
map<string, Material> loadMTL(string filepath);
...
//#endif
	...
	int nVerts;
	GLuint VAO = loadSimpleOBJ("../../3D_models/Cube/cube.obj", nVerts, glm::vec3(0.0,1.0,1.0));
	GLuint VAO2 = loadSimpleOBJ("../../3D_models/Cube/cube.obj", nVerts);
	GLuint VAO3 = loadSimpleOBJ("../../3D_models/Cube/cube.obj", nVerts, glm::vec3(1.0, 1.0, 0.0));
	glUniform1i(glGetUniformLocation(shader.ID, "tex_buffer"), 0);

	Mesh bola1, bola2, bola3;
	bola1.initialize(VAO, nVerts, &shader,glm::vec3(-2.75,0.0,0.0));
	bola2.initialize(VAO2, nVerts, &shader);
	bola3.initialize(VAO3, nVerts, &shader,glm::vec3(2.75, 0.0, 0.0));


	//Definindo as propriedades do material da superficie
	map<string, Material> materiais = loadMTL("../../3D_models/Cube/cube.mtl");
	string nomeMaterial = "Material"; // Substitua pelo nome do material desejado
	Material& material = materiais[nomeMaterial];
...
// Função para carregar materiais a partir de um arquivo .mtl
map<string, Material> loadMTL(string filepath) {
	...
	while (getline(file, line)) {
		...
		if (word == "newmtl") {
			...
			ss >> materials[currentMaterial].Ni;
		}
	}
